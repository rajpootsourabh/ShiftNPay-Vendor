...
function EnhancedTableToolbar(props) {
    ...
    const exportToExcel = () => {
        console.log("it will download the excel");
    ...
    return (
        <Toolbar sx={{ pl: { sm: 2 }, pr: { xs: 1, sm: 1 }, ...(numSelected > 0 && { bgcolor: (theme) => alpha(theme.palette.primary.main, theme.palette.action.activatedOpacity), }), }}>
            {numSelected > 0 ? (
                ...
                <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div">Employee</Typography>
            )}

            {numSelected > 0 ? (
                // <Tooltip title="Delete"><IconButton><MdDeleteForever /></IconButton></Tooltip>
                <Tooltip title="Delete"><Button color='success' sx={{ my: 1 }} variant="contained" onClick={exportToExcel}>Export to Excel</Button></Tooltip>
            ) : ('')}
        </Toolbar>
    );
}
...
export default function EnhancedTable({ emp, loading }) {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = emp.map((n) => n._id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - emp.length) : 0;

    const visibleRows = React.useMemo(() => stableSort(emp, getComparator(order, orderBy)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,), [order, orderBy, page, rowsPerPage],);
            ...
            <Paper sx={{ width: '100%', mb: 2 }}>
                ...
                <TableContainer>
                    <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
                        <EnhancedTableHead numSelected={selected.length} order={order} orderBy={orderBy} onSelectAllClick={handleSelectAllClick} onRequestSort={handleRequestSort} rowCount={emp.length} />
                        <TableBody>
                            ...
                            {visibleRows.map((row, index) => {
                                ...
                                return (
                                    <TableRow hover onClick={(event) => handleClick(event, row._id)} role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={row._id} selected={isItemSelected} sx={{ cursor: 'pointer' }}>
                ...
                <TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={emp.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} />
            </Paper>
